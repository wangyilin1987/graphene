...
int main(int argc, char** argv)
{
   try {
      ...
      app().set_default_data_dir(root / "eosio/nodgrap/data" );
      app().set_default_config_dir(root / "eosio/nodgrap/config" );
      http_plugin::set_defaults({
         .default_unix_socket_path = "",
         .default_http_port = 7777
      });

      if(!app().initialize<http_plugin, net_plugin>(argc, argv))
